...
#define POS_WEIGHT 20.0 // over weight ratio: must be a float
#define C_VAL 0.125 
...
private:
    ...
    SVMlight() {
        ...
        learn_parm->svm_costratio = POS_WEIGHT; // positive weight
        learn_parm->svm_costratio_unlab = 1.0; // unlabled weight
        ...
        learn_parm->svm_c = C_VAL; // 2^-5 ~ 2^+10
        learn_parm->type = REGRESSION;
        learn_parm->remove_inconsistent = 0; // -i 0 - Important
        kernel_parm->rbf_gamma = 0.0000000001;
        kernel_parm->coef_lin = 1;
        kernel_parm->coef_const = 1;
        kernel_parm->kernel_type = LINEAR; // 2 = RBF // -t 0 // -------------------- chris change
